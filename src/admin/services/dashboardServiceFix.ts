...
export const getDashboardStats = async () => {
  try {
    ...
    try {
        ...
        revenue = typeof revenueData === 'number' ? revenueData : revenueData.currentMonth;
    } catch (error) {
      logger.warn('Error getting revenue, using fallback:', error);
      // Fallback to mock revenue if Stripe API fails
...
// Generate fallback user growth data
const generateFallbackUserGrowth = (period: string): {
  period: string;
  new_users: number;
  cumulative_users: number;
}[] => {
...
export const getDashboardTrends = async (period: string): Promise<DashboardTrends> => {
  try {
    ...
    try {
      ...
      // Convert errorServiceTrend to our expected format
