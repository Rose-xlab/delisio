...
import { redisClient } from '../config/redis'; // Assuming redisClient is exported correctly
import { logger } from '../utils/logger';
import { Recipe, RecipeStep } from '../models/Recipe'; // Import necessary types
...
const recipeCache: Redis | Map<string, string> = redisClient instanceof Redis ? redisClient : new Map<string, string>();
const useRedis = redisClient instanceof Redis;

/**
 * Calculate progress percentage based on completed steps in a partial recipe
 */
function calculateProgress(recipe: Partial<Recipe> & { steps?: RecipeStep[] }): number {
  ...
  if (!recipe.steps || recipe.steps.length === 0) return 40;
 ...
 */
export const updatePartialRecipe = async (
    requestId: string,
    recipeData: Partial<Recipe>, // Base recipe data, potentially updated
    stepIndex?: number,          // Index of the step being updated (optional)
    imageUrl?: string | null     // URL of the image for the step (optional, null if failed?)
): Promise<boolean> => {
  const cacheKey = `recipe:${requestId}:partial`;
  try {
    let partialRecipe: any = null; // Initialize to null

    // 1. Get existing partial recipe from cache and parse it
    if (useRedis && redisClient) { // Ensure redisClient is usable
      try {
        const cachedData = await (recipeCache as Redis).get(cacheKey);
        if (cachedData) {
            partialRecipe = JSON.parse(cachedData);
        }
      } catch (redisError) {
         logger.error(`Redis GET error for ${cacheKey}: ${redisError instanceof Error ? redisError.message : redisError}`);
      ...
      const cachedString = (recipeCache as Map<string, string>).get(cacheKey);
      if (cachedString) {
        try {
        ...
        logger.error(`Cache misconfiguration for ${requestId}. Neither Redis nor Map seem active.`);
        return false;
    }
    ...
    if (!partialRecipe) {
      ...
      logger.info(`Initializing partial recipe cache for ${requestId}`);
      partialRecipe = {
      ...
      logger.info(`Merging partial recipe cache for ${requestId}`);
      ...
      partialRecipe = {
        ...
        steps: newStepsData.map((newStep: any, index: number) => {
    ...
    if (stepIndex !== undefined && partialRecipe.steps && partialRecipe.steps.length > stepIndex) {
      ...
      if (imageUrl !== undefined) {
        ...
        logger.info(`Updated step ${stepIndex} image URL for ${requestId}`);
      }
    }
    ...
    if (useRedis && redisClient) { // Ensure redisClient is usable
       try {
           ...
           await (recipeCache as Redis).set(cacheKey, dataToStore, 'EX', 3600);
       } catch (redisError) {
           logger.error(`Redis SET error for ${cacheKey}: ${redisError instanceof Error ? redisError.message : redisError}`);
      ...
      (recipeCache as Map<string, string>).set(cacheKey, dataToStore);
    ...
    logger.info(`Successfully updated partial recipe cache for ${requestId}, progress: ${partialRecipe.progress}%`);
    return true; // Indicate success
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error);
    logger.error(`Error updating partial recipe cache for ${requestId}: ${errorMsg}`, { stack: error instanceof Error ? error.stack : undefined });
