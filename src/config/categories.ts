...
export interface RecipeCategory {
   ...
   */
  export function detectRecipeCategory(title: string, ingredients: string[], steps: string[]): string {
    ...
    const normalizedSteps = steps.map(s => typeof s === 'string' ? s.toLowerCase() : 
      typeof s === 'object' && s && 'text' in s ? (s as any).text.toLowerCase() : '');
    ...
    const categoryScores = recipeCategories.map(category => {
      let score = 0;
   ...
   */
  export function getRelatedCategories(primaryCategory: string, title: string, ingredients: string[]): string[] {
    ...
    const otherCategories = recipeCategories.filter(c => c.id !== primaryCategory);
