 ...
 */
export const parseGptResponse = (gptResponse: string): ParsedRecipe => {
  try {
    ...
    throw new Error(`Failed to parse recipe: ${(error as Error).message}`);
  }
};
 ...
 */
function extractSteps(response: string): RecipeStep[] {
  let stepsSection = '';
 ...
 */
function extractNutrition(response: string): NutritionInfo {
 ...
 */
function extractServings(response: string): number {
 ...
 */
function extractIngredients(response: string): string[] {
