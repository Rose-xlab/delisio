...
import { detectRecipeCategory } from '../config/categories'; // Assuming this import is correct
 ...
 */
export function normalizeText(text: string | null | undefined): string { // Allow null/undefined input
 ...
 */
export function categorizeRecipe(recipe: Recipe): string {
  // Extract text from steps
  const stepsText = recipe.steps.map(step => step.text);

  // Detect category
  return detectRecipeCategory(recipe.title, recipe.ingredients, stepsText);
}
 ...
 */
export function parseTimeToMinutes(timeDescription: string | null | undefined): number | null {
...
/**
  ...
  */
 export function generateRecipeTags(recipe: Recipe): string[] {
...
/**
   ...
   */
  export function calculateBasicQualityScore(recipe: Recipe): number {
    let score = 0;

    if (recipe.title && recipe.title.length > 5) score += 1;
    if (recipe.servings && recipe.servings > 0) score += 0.5;

    const ingredientsList: string[] = recipe.ingredients;
    if (ingredientsList && ingredientsList.length > 0) {
      score += 1;
      const currentIngredientsLength = ingredientsList.length;
      if (currentIngredientsLength >= 5) score += 0.5;
      if (currentIngredientsLength >= 10) score += 0.5;
      const detailedIngredientsCount = ingredientsList.filter(i =>
        /\d/.test(i) ||
        /cup|tbsp|tsp|teaspoon|tablespoon|ounce|oz|pound|lb|gram|g|kg|ml|l|pinch|dash|clove/i.test(i)
      ).length;
    ...
    const stepsList: RecipeStep[] = recipe.steps;
    if (stepsList && stepsList.length > 0) {
      score += 1;
      const currentStepsLength = stepsList.length;
      if (currentStepsLength >= 3) score += 0.5;
      if (currentStepsLength >= 5) score += 0.5;
    ...
    if (recipe.nutrition) {
      ...
      if (recipe.nutrition.calories && recipe.nutrition.calories > 0) nutritionDetailScore += 0.25;
      if (recipe.nutrition.protein && parseFloat(recipe.nutrition.protein) >= 0) nutritionDetailScore += 0.25;
      if (recipe.nutrition.carbs && parseFloat(recipe.nutrition.carbs) >= 0) nutritionDetailScore += 0.25;
      if (recipe.nutrition.fat && parseFloat(recipe.nutrition.fat) >= 0) nutritionDetailScore += 0.25;
      score += nutritionDetailScore;
    }

    let timeScore = 0;
    if (recipe.prepTime && recipe.prepTime > 0) timeScore += 0.25;
    if (recipe.cookTime && recipe.cookTime > 0) timeScore += 0.25;
    if (recipe.totalTime && recipe.totalTime > 0) timeScore += 0.25;
