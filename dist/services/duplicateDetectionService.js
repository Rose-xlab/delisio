...
exports.generateSimilarityHash = exports.mergeRecipes = exports.checkForDuplicates = void 0;
 ...
 */
const checkForDuplicates = async (recipe) => {
    try {
        logger_1.logger.info(`Checking for duplicates of recipe: ${recipe.title}`);
        // Generate a similarity hash for the recipe
        const similarityHash = (0, exports.generateSimilarityHash)(recipe);
        ...
        const { data: potentialDuplicates, error } = await supabase_1.supabase
            .from('recipes')
            .select('*')
            .or(`title.ilike.%${normalizedTitle}%,similarity_hash.eq.${similarityHash}`)
            .limit(10);
        if (error) {
            logger_1.logger.error('Error querying for potential duplicates:', error);
            return {
                isDuplicate: false,
                similarityScore: 0,
        ...
        if (!potentialDuplicates || potentialDuplicates.length === 0) {
            logger_1.logger.info('No potential duplicates found');
            return {
                isDuplicate: false,
                similarityScore: 0,
                existingRecipeId: null,
        ...
        for (const potentialDuplicate of potentialDuplicates) {
            const dbRecipe = {
                ...
                steps: (potentialDuplicate.steps || []),
                nutrition: (potentialDuplicate.nutrition || {}),
            ...
            if (dbRecipe.id === recipe.id)
                continue;
            const similarityDetails = calculateRecipeSimilarity(recipe, dbRecipe, newRecipeIngredientAnalysis);
            const combinedScore = (similarityDetails.titleSimilarity * 0.3 +
        ...
        logger_1.logger.info(`Duplicate check result for ${recipe.title}: ${isDuplicate ? 'IS' : 'NOT'} a duplicate. Similarity score: ${highestSimilarityScore.toFixed(2)}`);
        return {
            isDuplicate,
        ...
        logger_1.logger.error('Error checking for duplicates:', error);
        return {
            isDuplicate: false,
            similarityScore: 0,
            existingRecipeId: null,
 ...
 */
const mergeRecipes = async (newRecipe, existingRecipeId) => {
    try {
        logger_1.logger.info(`Merging recipe ${newRecipe.title} with existing recipe ID ${existingRecipeId}`);
        // Get the existing recipe
        const { data: existingRecipeData, error } = await supabase_1.supabase
            .from('recipes')
            .select('*')
            .eq('id', existingRecipeId)
            .single();
        if (error || !existingRecipeData) {
            logger_1.logger.error('Error fetching existing recipe for merge:', error || 'No data returned');
        ...
        const existingRecipe = {
            ...
            steps: (existingRecipeData.steps || []),
            nutrition: (existingRecipeData.nutrition || {}),
        ...
        // Perform the merge
        const mergedTitle = existingRecipe.title.length > newRecipe.title.length
            ? existingRecipe.title
            : newRecipe.title;
        const allIngredients = [...existingRecipe.ingredients, ...newRecipe.ingredients];
        const uniqueIngredients = Array.from(new Set(allIngredients.map(normalizeText)))
            .map(normalized => {
            const matching = allIngredients.filter(ing => normalizeText(ing) === normalized);
            return matching.length > 0
                ? matching.reduce((a, b) => (b.length > a.length ? b : a), matching[0])
                : '';
        })
            .filter(ing => ing !== '');
        const mergedSteps = existingRecipe.steps.length >= newRecipe.steps.length
            ? existingRecipe.steps
            : newRecipe.steps;
        const existingHasNutrition = !!existingRecipe.nutrition.calories &&
            !!existingRecipe.nutrition.protein &&
            !!existingRecipe.nutrition.fat &&
            !!existingRecipe.nutrition.carbs;
        const newHasNutrition = !!newRecipe.nutrition.calories &&
            !!newRecipe.nutrition.protein &&
            !!newRecipe.nutrition.fat &&
            !!newRecipe.nutrition.carbs;
        const mergedNutrition = existingHasNutrition
            ? existingRecipe.nutrition
            : newHasNutrition
                ? newRecipe.nutrition
                : existingRecipe.nutrition;
        const mergedPrepTime = mergeTimeValues(existingRecipe.prepTime, newRecipe.prepTime);
        const mergedCookTime = mergeTimeValues(existingRecipe.cookTime, newRecipe.cookTime);
        const mergedTotalTime = mergeTimeValues(existingRecipe.totalTime, newRecipe.totalTime);
        const mergedRecipe = {
        ...
        await supabase_1.supabase
            ...
            steps: mergedRecipe.steps,
            nutrition: mergedRecipe.nutrition,
            ...
            .eq('id', existingRecipeId);
        logger_1.logger.info(`Successfully merged recipes. Keeping ID ${existingRecipeId}`);
        return mergedRecipe;
    }
    catch (error) {
        logger_1.logger.error('Error merging recipes:', error);
        throw new Error(`Failed to merge recipes: ${error.message}`);
    }
};
...
const generateSimilarityHash = (recipe) => {
    ...
    return (0, crypto_1.createHash)('sha256').update(Buffer.from(hashInput)).digest('hex');
};
...
function calculateRecipeSimilarity(recipe1, recipe2, recipe1IngredientAnalysis) {
    const titleSimilarity = calculateJaccardSimilarity(normalizeText(recipe1.title).split(' '), normalizeText(recipe2.title).split(' '));
    ...
    const ingredientSimilarity = calculateJaccardSimilarity(analysis1.normalizedIngredients, analysis2.normalizedIngredients);
...
function mergeTimeValues(time1, time2) {
